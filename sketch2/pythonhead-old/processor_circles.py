...
def handleFrame(frame):
    ...
    circles = cv.HoughCircles(img, cv.HOUGH_GRADIENT, 1, 10, np.array([]), 150, 30, minRadius=5, maxRadius=50)
    ...
    if circles is not None and len(circles)>0: # Check if circles have been found and only then iterate over these and add them to the image
        a, b, c = circles.shape
        for i in range(b):
            cv.circle(cimg, (int(circles[0][i][0]), int(circles[0][i][1])), int(circles[0][i][2]), (0, 0, 255), 3, cv.LINE_AA)
    ...
    if circles is not None and len(circles)>0: # Check if circles have been found and only then iterate over these and add them to the image
        a, b, c = circles.shape
        for i in range(b):
            o.append( dict(cx=float(circles[0][i][0]), cy=float(circles[0][i][1]), r=float(circles[0][i][2])) )
