...
def handleFrame(frame):
    ...
    out = dict(frame=n, mean_background_px=backx,mean_background_py=backy, mean_background=np.mean(fgmask))
    ...
    if (arguments.compute_contours):
        ...
        im2, contours  = cv.findContours(img_as_ubyte(fgmask),cv.RETR_LIST,cv.CHAIN_APPROX_SIMPLE)

        data = [[[x/frame.shape[1],y/frame.shape[0]] for x,y in chunker(np.concatenate(q).ravel().tolist(),2)] for q in im2 if cv.contourArea(q)>arguments.area_threshold]
        out["contours"] = data
